...
- [ ] T001 Crear migración `backend/portfolio/src/main/resources/db/migration/V3__create_position_table.sql` — tabla `position` (id UUID PK, investor_id UUID NOT NULL, symbol VARCHAR(20) NOT NULL, name VARCHAR(100) NOT NULL, quantity INT NOT NULL CHECK > 0, avg_buy_price DECIMAL(18,2) NOT NULL CHECK > 0, currency VARCHAR(3) DEFAULT 'COP', updated_at, created_at); constraint UNIQUE(investor_id, symbol); índice `idx_position_investor(investor_id)`
...
- [ ] T005 [P] [US1] Escribir `PortfolioControllerTest` en `src/test/java/.../controller/PortfolioControllerTest.java` con `@WebMvcTest`: test `GET /portfolio/holdings` → 200 con lista; test portafolio vacío → lista vacía; test investor no puede ver portafolio de otro
- [ ] T006 [P] [US1] Escribir `PositionCalculatorTest` en `src/test/java/.../service/PositionCalculatorTest.java` — test precio promedio ponderado con 2 compras; test P&L positivo/negativo/cero; test pnlPercent cuando avgBuyPrice == currentPrice → 0.00
- [ ] T007 [P] [US1] Escribir `PortfolioServiceTest` con Mockito: test que `StockSnapshotService` es llamado por cada posición; test fallback cuando snapshot no existe → currentPrice = avgBuyPrice, pnlAmount = 0

### Implementación — US1

- [ ] T008 [P] [US1] Crear `PositionDto` en `dto/PositionDto.java` — symbol, name, quantity, avgBuyPrice, currentPrice, positionValue, pnlAmount, pnlPercent, dayChange, dayChangePct, currency (todos BigDecimal nullable para currentPrice/dayChange cuando no hay snapshot)
- [ ] T009 [US1] Implementar `PortfolioService.getHoldings(UUID investorId)` en `service/PortfolioService.java` — (1) cargar posiciones con `positionRepository.findByInvestorId()`; (2) para cada posición: obtener snapshot via `StockSnapshotService.findBySymbol()` in-process; si null → currentPrice=avgBuyPrice, dayChange=null; (3) calcular positionValue, pnlAmount, pnlPercent con `PositionCalculator`; retornar List<PositionDto> (depende de T004)
- [ ] T010 [US1] Implementar `PortfolioController` en `controller/PortfolioController.java` con `GET /portfolio/holdings` — extrae investorId del JWT, delega a `portfolioService.getHoldings()`, retorna 200
...
- [ ] T011 [P] [US2] Agregar test a `PortfolioControllerTest`: `GET /portfolio/summary` → 200 con campos correctos; portafolio vacío → todos cero
- [ ] T012 [P] [US2] Agregar test a `PortfolioServiceTest`: `getSummary()` suma correctamente con 2 posiciones; `totalPnlPct` = 0 cuando totalCost = 0

### Implementación — US2

- [ ] T013 [P] [US2] Crear `PortfolioSummaryDto` en `dto/PortfolioSummaryDto.java` — investorId, totalValue, totalCost, totalPnl, totalPnlPct, totalDayChange, positionCount, currency
- [ ] T014 [US2] Implementar `PortfolioService.getSummary(UUID investorId)` — reutilizar `getHoldings()` (T009); agregar en memoria: totalValue=SUM(positionValue), totalCost=SUM(avgBuyPrice×qty), totalPnl=totalValue-totalCost, totalPnlPct=totalPnl/totalCost×100 (0 si cost=0), totalDayChange=SUM(dayChange×qty ignorando nulls); retornar PortfolioSummaryDto
- [ ] T015 [US2] Agregar `GET /portfolio/summary` en `PortfolioController` (depende de T014)
...
- [ ] T016 Verificar constraint UNIQUE(investor_id, symbol) — intentar insertar posición duplicada en test lanza excepción de DB; la capa de servicio debe usar `findByInvestorIdAndSymbol()` + update en vez de doble insert
- [ ] T017 [P] Ejecutar suite: `./mvnw test -pl backend/portfolio`
