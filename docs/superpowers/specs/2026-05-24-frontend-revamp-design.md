...
- Logo: `<img src="unbosque-logo.png" height="40px">` + 1px `var(--line)` divider + italic serif wordmark "Acciones El Bosque" with "El Bosque" in `var(--moss)`
...
- Right: `var(--paper)`, logo `height: 60px` absolute top-right (top: 36px, right: 48px)
...
Copy `design_handoff_frontend_revamp/prototype/assets/unbosque-logo.png` → `frontend/public/unbosque-logo.png`. Used as `src="unbosque-logo.png"` (existing SVG path stays for fallback).
